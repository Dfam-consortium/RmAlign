# telib/sequences/twobit.py
from __future__ import annotations

import io
import struct
from dataclasses import dataclass
from typing import Dict, Iterable, Optional, Tuple

from .base import SequenceSource, reverse_complement

__all__ = ["TwoBitSequenceSource"]

# UCSC 2bit constants
TWOBIT_SIGNATURE = 0x1A412743
DNA_DECODE = {0: "T", 1: "C", 2: "A", 3: "G"}

# Fast byte → 4-bases lookup table (each entry is a 4-byte ASCII chunk)
_DNA_DECODE_BYTES = (b"T", b"C", b"A", b"G")
_LUT_4BASES = [b""] * 256
for _b in range(256):
    _LUT_4BASES[_b] = (
        _DNA_DECODE_BYTES[(_b >> 6) & 3]
        + _DNA_DECODE_BYTES[(_b >> 4) & 3]
        + _DNA_DECODE_BYTES[(_b >> 2) & 3]
        + _DNA_DECODE_BYTES[_b & 3]
    )


@dataclass
class _TwoBitEntry:
    name: str
    offset: int  # absolute file offset of the record (dnaSize field)


@dataclass
class _TwoBitMeta:
    dna_size: int
    n_starts: Tuple[int, ...]
    n_sizes: Tuple[int, ...]
    packed_offset: int  # absolute file offset where packed DNA starts


@dataclass
class TwoBitSequenceSource(SequenceSource):
    """
    Minimal UCSC 2bit reader with v0/v1 index support and hybrid decoding.

    - Detects endianness from the magic (0x1A412743).
    - Reads header {version:u32, sequenceCount:u32, reserved:u32}.
    - Index entries:
        v0: nameLen:u8, name:bytes, offset:u32
        v1: nameLen:u8, name:bytes, offset:u64
    - Record layout (common for v0 and v1):
        dnaSize:u32
        nBlockCount:u32, nBlockStarts[nBlockCount]:u32, nBlockSizes[nBlockCount]:u32
        maskBlockCount:u32, maskStarts[maskCount]:u32, maskSizes[maskCount]:u32   (ignored for case)
        reserved:u32
        packedDNA: ceil(dnaSize/4) bytes (T=0, C=1, A=2, G=3)
    - Returns uppercase sequences; N blocks applied.
    - Coordinates for get(): 1-based, fully-closed; reverse-complement if strand == '-'.
    - Caching policy:
        * Full-sequence cache on demand (first large request promotes to full decode).
        * Small-window requests decode only the needed slice without populating the full cache.
    """
    path: str  # path to .2bit file

    # Private attributes populated during __post_init__:
    #   _endian: str ('>' or '<')
    #   _version: int (0 or 1)
    #   _entries: Dict[str, _TwoBitEntry]
    #   _meta: Dict[str, _TwoBitMeta]
    #   _cache: Dict[str, str]  # full decoded sequences

    def __post_init__(self):
        self._entries: Dict[str, _TwoBitEntry] = {}
        self._cache: Dict[str, str] = {}
        self._meta: Dict[str, _TwoBitMeta] = {}

        with open(self.path, "rb") as f:
            # --- Magic / endianness ---
            sig_bytes = f.read(4)
            if len(sig_bytes) != 4:
                raise ValueError("Not a 2bit file (too short for signature)")
            sig_be = struct.unpack(">I", sig_bytes)[0]
            sig_le = struct.unpack("<I", sig_bytes)[0]

            if sig_be == TWOBIT_SIGNATURE:
                endian = ">"
            elif sig_le == TWOBIT_SIGNATURE:
                endian = "<"
            else:
                raise ValueError("Not a 2bit file (bad signature)")
            self._endian = endian

            # --- Header ---
            hdr = f.read(12)
            if len(hdr) != 12:
                raise ValueError("Not a 2bit file (truncated header)")
            version, count, _reserved = struct.unpack(f"{endian}III", hdr)
            if version not in (0, 1):
                raise ValueError(f"Unsupported 2bit version {version}")
            self._version = int(version)

            # --- Index ---
            for _ in range(count):
                b = f.read(1)
                if not b:
                    raise ValueError("Truncated 2bit index (name length)")
                (name_len,) = struct.unpack("B", b)  # 1 byte, endianness irrelevant

                name_b = f.read(name_len)
                if len(name_b) != name_len:
                    raise ValueError("Truncated 2bit index (name)")
                name = name_b.decode("ascii")

                if version == 0:
                    off_b = f.read(4)
                    if len(off_b) != 4:
                        raise ValueError("Truncated 2bit index (offset32)")
                    (offset,) = struct.unpack(f"{endian}I", off_b)
                else:
                    off_b = f.read(8)
                    if len(off_b) != 8:
                        raise ValueError("Truncated 2bit index (offset64)")
                    (offset,) = struct.unpack(f"{endian}Q", off_b)

                self._entries[name] = _TwoBitEntry(name=name, offset=int(offset))

    # ----------------------------
    # SequenceSource implementation
    # ----------------------------

    def has(self, seq_id: str) -> bool:
        return seq_id in self._entries

    def ids(self) -> Iterable[str]:
        return self._entries.keys()

    def length(self, seq_id: str) -> int:
        s = self._cache.get(seq_id)
        if s is not None:
            return len(s)
        return self._ensure_meta(seq_id).dna_size

    # ----------------------------
    # Internal helpers
    # ----------------------------

    def _ensure_meta(self, seq_id: str) -> _TwoBitMeta:
        m = self._meta.get(seq_id)
        if m is not None:
            return m
        e = self._entries.get(seq_id)
        if e is None:
            raise KeyError(f"2bit: sequence id {seq_id!r} not found")

        endian = self._endian
        with open(self.path, "rb") as f:
            f.seek(e.offset)

            # dnaSize
            b = f.read(4)
            if len(b) != 4:
                raise ValueError("Truncated 2bit record (dnaSize)")
            dna_size = struct.unpack(f"{endian}I", b)[0]

            # N blocks
            b = f.read(4)
            if len(b) != 4:
                raise ValueError("Truncated 2bit record (nBlockCount)")
            n_block_count = struct.unpack(f"{endian}I", b)[0]

            if n_block_count:
                nb = f.read(4 * n_block_count)
                if len(nb) != 4 * n_block_count:
                    raise ValueError("Truncated 2bit record (nStarts)")
                n_starts = struct.unpack(f"{endian}{n_block_count}I", nb)

                nb = f.read(4 * n_block_count)
                if len(nb) != 4 * n_block_count:
                    raise ValueError("Truncated 2bit record (nSizes)")
                n_sizes = struct.unpack(f"{endian}{n_block_count}I", nb)
            else:
                n_starts = ()
                n_sizes = ()

            # Mask blocks (ignored for case)
            b = f.read(4)
            if len(b) != 4:
                raise ValueError("Truncated 2bit record (maskBlockCount)")
            mask_count = struct.unpack(f"{endian}I", b)[0]
            if mask_count:
                # skip maskStarts + maskSizes
                f.seek(8 * mask_count, io.SEEK_CUR)

            # reserved
            f.seek(4, io.SEEK_CUR)

            # Start of packed DNA
            packed_offset = f.tell()

        m = _TwoBitMeta(
            dna_size=int(dna_size),
            n_starts=tuple(int(x) for x in n_starts),
            n_sizes=tuple(int(x) for x in n_sizes),
            packed_offset=int(packed_offset),
        )
        self._meta[seq_id] = m
        return m

    def _apply_n_blocks_in_place(self, buf: bytearray, window_start0: int, meta: _TwoBitMeta) -> None:
        """
        buf holds window bases (0-based relative to window_start0).
        window_start0 is 0-based genomic start of buf[0].
        """
        ws = window_start0
        we = ws + len(buf)
        for st, sz in zip(meta.n_starts, meta.n_sizes):
            lo = max(st, ws)
            hi = min(st + sz, we)
            if lo < hi:
                a = lo - ws
                b = hi - ws
                buf[a:b] = b"N" * (b - a)

    def _decode_window(self, seq_id: str, start: int, end: int) -> str:
        """
        Decode only [start, end] (1-based, fully-closed) for seq_id.
        Does not populate the full-sequence cache.
        """
        meta = self._ensure_meta(seq_id)
        if start < 1 or end < start:
            raise ValueError("invalid range")
        if start > meta.dna_size:
            return ""
        end = min(end, meta.dna_size)

        start0 = start - 1
        end0 = end - 1

        # Which packed bytes cover [start0, end0]?
        byte_lo = start0 // 4
        byte_hi = end0 // 4
        nbytes = byte_hi - byte_lo + 1

        with open(self.path, "rb") as f:
            f.seek(meta.packed_offset + byte_lo)
            packed = f.read(nbytes)
            if len(packed) != nbytes:
                raise ValueError("Truncated 2bit record (window read)")

        # LUT-decode the packed bytes to bases, then trim exactly to [start0, end0]
        chunk_base0 = byte_lo * 4
        seq_bytes = b"".join(_LUT_4BASES[b] for b in packed)
        a = start0 - chunk_base0
        b = a + (end0 - start0 + 1)
        window = bytearray(seq_bytes[a:b])

        # Apply N blocks overlapping this window
        self._apply_n_blocks_in_place(window, start0, meta)

        return window.decode("ascii")

    def _decode_sequence_bytes(self, seq_id: str) -> str:
        """
        Decode the full sequence for seq_id and return as a string.
        Populates the full-sequence cache via _ensure_decoded().
        """
        meta = self._ensure_meta(seq_id)

        with open(self.path, "rb") as f:
            f.seek(meta.packed_offset)
            nbytes = (meta.dna_size + 3) // 4
            packed = f.read(nbytes)
            if len(packed) != nbytes:
                raise ValueError("Truncated 2bit record (packed DNA)")

        # Fast LUT decode for the whole sequence
        seq_bytes = b"".join(_LUT_4BASES[b] for b in packed)[: meta.dna_size]
        buf = bytearray(seq_bytes)

        # Apply N blocks quickly
        for st, sz in zip(meta.n_starts, meta.n_sizes):
            lo = max(0, st)
            hi = min(st + sz, meta.dna_size)
            if lo < hi:
                buf[lo:hi] = b"N" * (hi - lo)

        return buf.decode("ascii")

    def _ensure_decoded(self, seq_id: str) -> str:
        s = self._cache.get(seq_id)
        if s is None:
            s = self._decode_sequence_bytes(seq_id)
            if not isinstance(s, str):
                raise RuntimeError("2bit internal error: decoded sequence is not a string")
            self._cache[seq_id] = s
        return s

    def get(self, seq_id: str, start: int, end: int, strand: str = "+") -> str:
        # Normalize & validate (1-based, fully closed)
        if start < 1 or end < start:
            raise ValueError("invalid range")

        # If we already have the full sequence cached, slice it.
        full = self._cache.get(seq_id)
        if full is not None:
            end = min(end, len(full))
            if start > end:
                return ""
            frag = full[start - 1 : end]
            return frag if strand != "-" else reverse_complement(frag)

        # Decide window vs full: small windows read just what’s needed
        meta = self._ensure_meta(seq_id)
        span = min(end, meta.dna_size) - start + 1
        if span <= 0:
            return ""

        # Heuristic threshold: window-decode if request is small; else promote to full decode
        # Tune as desired. Here: ≤ 1 MiB or ≤ 5% of the sequence.
        small = (span <= 1_000_000) or (span <= max(100_000, meta.dna_size // 20))

        if small:
            frag = self._decode_window(seq_id, start, end)
            return frag if strand != "-" else reverse_complement(frag)

        # Large slice → decode whole sequence once and cache
        s = self._ensure_decoded(seq_id)
        end = min(end, len(s))
        if start > end:
            return ""
        frag = s[start - 1 : end]
        return frag if strand != "-" else reverse_complement(frag)

